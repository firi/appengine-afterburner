...
class LogMessage:
    """
    A single log message, either from application code or system-generated.
    ...
    def fetch_request_logs(self, cursor: Cursor) -> tuple[list[RequestLog], Cursor | None]:
        ...
        if status_filter:
            ...
            if status_filter == '2xx':
                ...
                app_log_filters.append('(httpRequest.status>=200 AND httpRequest.status<300)')
            elif status_filter == '3xx':
                request_filters.append('(protoPayload.status>=300 AND protoPayload.status<400)')
                app_log_filters.append('(httpRequest.status>=300 AND httpRequest.status<400)')
            elif status_filter == '4xx':
                request_filters.append('(protoPayload.status>=400 AND protoPayload.status<500)')
                app_log_filters.append('(httpRequest.status>=400 AND httpRequest.status<500)')
            elif status_filter == '5xx':
                request_filters.append('(protoPayload.status>=500 AND protoPayload.status<600)')
                app_log_filters.append('(httpRequest.status>=500 AND httpRequest.status<600)')
            elif status_filter == 'errors':
                request_filters.append('(protoPayload.status>=400)')
                app_log_filters.append('(httpRequest.status>=400)')
        if path_filter:
            # Use substring matching for simple path filtering
            request_filters.append(f'protoPayload.resource:"{path_filter}"')
            app_log_filters.append(f'httpRequest.requestUrl:"{path_filter}"')
        # App logs can have a severity
        if min_severity:
            app_log_filters.append(f'severity>="{min_severity}"')
        ...
        # synthetic requests so they show up. Missing request logs can happen
        # if there are request log sinks configured in Cloud Logging.
